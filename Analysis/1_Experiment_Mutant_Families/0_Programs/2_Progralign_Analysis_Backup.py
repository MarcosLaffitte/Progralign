...
constructionDict_rGT = dict()                # expand over random guide tree with ambiguous edges
constructionDict_rGT_notAE = dict()          # expand over random guide tree without ambiguous edges
# trimm-experiments
constructionDict_trimm = dict()              # trimm with ambiguous edges
constructionDict_trimm_notAE = dict()        # trimm without ambiguous edges
constructionDict_trimm_rGT = dict()          # trimm over random guide tree with ambiguous edges
constructionDict_trimm_rGT_notAE = dict()    # trimm over random guide tree without ambiguous edges
...
# function: get similarities between primal and consensus graphs ---------------
def getSimilarities(somePrimal, someConsensusGraphs):
    # local variables
    similaritySSP = 0
    finalSimilarites = []
    G1 = None
    ...
    # loop obtaninng similarities
    for eachConsensus in someConsensusGraphs:
        # make copy of graphs apropiate for graph kernels
        G1 = deepcopy(condensedLabelToStr(somePrimal))
        G2 = deepcopy(condensedLabelToStr(eachConsensus))
        # kernel evaluation
        kernelMatrixSSP = sspk.structuralspkernel(G1, G2, parallel = None, verbose = False,
        ...
        similaritySSP = kernelMatrixSSP[0][1]
        # save similarities
        finalSimilarites.append(similaritySSP)
    # end of function
    return(finalSimilarites)
...
# initialize dictionary for experiment with random tree
constructionDict_rGT = deepcopy(constructionDict)
...
# initialize dictinoaries for experiments with and without ambiguous edges
# expand-experiments
...
# task message
print("* Building random guide tree ...")
...
# construction of random guide tree
randDendrogram, randGuideTree, randContainedGraphs = getGuideTree(randomClusteringGraphs[0], guideTreeMeasure)
constructionDict_rGT["GTMeasure"] = guideTreeMeasure
constructionDict_rGT["Dendrogram"] = randDendrogram
constructionDict_rGT["GuideTree"] = deepcopy(randGuideTree)
constructionDict_rGT["Contained"] = deepcopy(randContainedGraphs)


# construction of random guide tree dictionaries
constructionDict_rGT_notAE = deepcopy(constructionDict_rGT)
constructionDict_trimm_rGT = deepcopy(constructionDict_rGT)
constructionDict_trimm_rGT_notAE = deepcopy(constructionDict_rGT)
...
constructionDict["Primal"] = deepcopy(primal)
constructionDict["GapsAndAEs"] = (getGaps(constructionDict["Alignment"], constructionDict["Existence"][constructionDict["Dendrogram"]], constructionDict["AllLeaves"]),
...
constructionDict_notAE["Primal"] = deepcopy(primal)
constructionDict_notAE["GapsAndAEs"] = (getGaps(constructionDict_notAE["Alignment"], constructionDict_notAE["Existence"][constructionDict_notAE["Dendrogram"]], constructionDict_notAE["AllLeaves"]),
                                        len(constructionDict_notAE["AmbiguousEdges"][constructionDict_notAE["Dendrogram"]]))
...
constructionDict_trimm["Primal"] = deepcopy(primal)
constructionDict_trimm["GapsAndAEs"] = (getGaps(constructionDict_trimm["Alignment"], constructionDict_trimm["Existence"][constructionDict_trimm["Dendrogram"]], constructionDict_trimm["AllLeaves"]),
                                        len(constructionDict_trimm["AmbiguousEdges"][constructionDict_trimm["Dendrogram"]]))
...
constructionDict_trimm_notAE["Primal"] = deepcopy(primal)
constructionDict_trimm_notAE["GapsAndAEs"] = (getGaps(constructionDict_trimm_notAE["Alignment"], constructionDict_trimm_notAE["Existence"][constructionDict_trimm_notAE["Dendrogram"]], constructionDict_trimm_notAE["AllLeaves"]),
                                              len(constructionDict_trimm_notAE["AmbiguousEdges"][constructionDict_trimm_notAE["Dendrogram"]]))


# task message
print("\n")
print("* Progressive graph alignment - Random Tree - expand - with ambiguous edges")
# progressive graph alignment
alignmentResults = PGA(theType, deepcopy(constructionDict_rGT["GuideTree"]),
                       constructionDict_rGT, constructionDict_rGT["Dendrogram"],
                       constructionDict_rGT["Contained"], requestedScore,
                       holdV = respectVLabels, holdE = respectELabels,
                       holdAmbEdges = ambiguousEdges,
                       algorithm = MCSalgorithm)
# unpack results
constructionDict_rGT["Alignment"] = deepcopy(alignmentResults[0])
constructionDict_rGT["allAlignments"] = deepcopy(alignmentResults[1])
constructionDict_rGT["Existence"] = deepcopy(alignmentResults[2])
constructionDict_rGT["ScoreInfo"] = deepcopy(alignmentResults[3])
constructionDict_rGT["AmbiguousEdges"] = deepcopy(alignmentResults[4])
constructionDict_rGT["MapToRoot"] = deepcopy(alignmentResults[5])
constructionDict_rGT["AllLeaves"] = deepcopy(alignmentResults[6])
constructionDict_rGT["AllLeavesGraphs"] = deepcopy(alignmentResults[7])
constructionDict_rGT["Time"] = deepcopy(alignmentResults[8])
constructionDict_rGT["Primal"] = deepcopy(primal)
constructionDict_rGT["GapsAndAEs"] = (getGaps(constructionDict_rGT["Alignment"], constructionDict_rGT["Existence"][constructionDict_rGT["Dendrogram"]], constructionDict_rGT["AllLeaves"]),
                                      len(constructionDict_rGT["AmbiguousEdges"][constructionDict_rGT["Dendrogram"]]))


# task message
print("\n")
print("* Progressive graph alignment - Random Tree - expand - without ambiguous edges")
# progressive graph alignment
alignmentResults = PGA(theType, deepcopy(constructionDict_rGT["GuideTree"]),
                       constructionDict_rGT, constructionDict_rGT["Dendrogram"],
                       constructionDict_rGT["Contained"], requestedScore,
                       ...
                       algorithm = MCSalgorithm)
# unpack results
constructionDict_rGT_notAE["Alignment"] = deepcopy(alignmentResults[0])
constructionDict_rGT_notAE["allAlignments"] = deepcopy(alignmentResults[1])
constructionDict_rGT_notAE["Existence"] = deepcopy(alignmentResults[2])
constructionDict_rGT_notAE["ScoreInfo"] = deepcopy(alignmentResults[3])
constructionDict_rGT_notAE["AmbiguousEdges"] = deepcopy(alignmentResults[4])
constructionDict_rGT_notAE["MapToRoot"] = deepcopy(alignmentResults[5])
constructionDict_rGT_notAE["AllLeaves"] = deepcopy(alignmentResults[6])
constructionDict_rGT_notAE["AllLeavesGraphs"] = deepcopy(alignmentResults[7])
constructionDict_rGT_notAE["Time"] = deepcopy(alignmentResults[8])
constructionDict_rGT_notAE["Primal"] = deepcopy(primal)
constructionDict_rGT_notAE["GapsAndAEs"] = (getGaps(constructionDict_rGT_notAE["Alignment"], constructionDict_rGT_notAE["Existence"][constructionDict_rGT_notAE["Dendrogram"]], constructionDict_rGT_notAE["AllLeaves"]),
                                            len(constructionDict_rGT_notAE["AmbiguousEdges"][constructionDict_rGT_notAE["Dendrogram"]]))


# task message
print("\n")
print("* Progressive graph alignment - Random Tree - trimm - with ambiguous edges")
# progressive graph alignment
alignmentResults = PGA(theType, deepcopy(constructionDict_rGT["GuideTree"]),
                       constructionDict_rGT, constructionDict_rGT["Dendrogram"],
                       constructionDict_rGT["Contained"], requestedScore,
                       ...
                       algorithm = "trimm")
# unpack results
constructionDict_trimm_rGT["Alignment"] = deepcopy(alignmentResults[0])
constructionDict_trimm_rGT["allAlignments"] = deepcopy(alignmentResults[1])
constructionDict_trimm_rGT["Existence"] = deepcopy(alignmentResults[2])
constructionDict_trimm_rGT["ScoreInfo"] = deepcopy(alignmentResults[3])
constructionDict_trimm_rGT["AmbiguousEdges"] = deepcopy(alignmentResults[4])
constructionDict_trimm_rGT["MapToRoot"] = deepcopy(alignmentResults[5])
constructionDict_trimm_rGT["AllLeaves"] = deepcopy(alignmentResults[6])
constructionDict_trimm_rGT["AllLeavesGraphs"] = deepcopy(alignmentResults[7])
constructionDict_trimm_rGT["Time"] = deepcopy(alignmentResults[8])
constructionDict_trimm_rGT["Primal"] = deepcopy(primal)
constructionDict_trimm_rGT["GapsAndAEs"] = (getGaps(constructionDict_trimm_rGT["Alignment"], constructionDict_trimm_rGT["Existence"][constructionDict_trimm_rGT["Dendrogram"]], constructionDict_trimm_rGT["AllLeaves"]),
                                            len(constructionDict_trimm_rGT["AmbiguousEdges"][constructionDict_trimm_rGT["Dendrogram"]]))


# task message
print("\n")
print("* Progressive graph alignment - Random Tree - trimm - without ambiguous edges")
# progressive graph alignment
alignmentResults = PGA(theType, deepcopy(constructionDict_rGT["GuideTree"]),
                       constructionDict_rGT, constructionDict_rGT["Dendrogram"],
                       constructionDict_rGT["Contained"], requestedScore,
                       ...
                       algorithm = "trimm")
# unpack results
constructionDict_trimm_rGT_notAE["Alignment"] = deepcopy(alignmentResults[0])
constructionDict_trimm_rGT_notAE["allAlignments"] = deepcopy(alignmentResults[1])
constructionDict_trimm_rGT_notAE["Existence"] = deepcopy(alignmentResults[2])
constructionDict_trimm_rGT_notAE["ScoreInfo"] = deepcopy(alignmentResults[3])
constructionDict_trimm_rGT_notAE["AmbiguousEdges"] = deepcopy(alignmentResults[4])
constructionDict_trimm_rGT_notAE["MapToRoot"] = deepcopy(alignmentResults[5])
constructionDict_trimm_rGT_notAE["AllLeaves"] = deepcopy(alignmentResults[6])
constructionDict_trimm_rGT_notAE["AllLeavesGraphs"] = deepcopy(alignmentResults[7])
constructionDict_trimm_rGT_notAE["Time"] = deepcopy(alignmentResults[8])
constructionDict_trimm_rGT_notAE["Primal"] = deepcopy(primal)
constructionDict_trimm_rGT_notAE["GapsAndAEs"] = (getGaps(constructionDict_trimm_rGT_notAE["Alignment"], constructionDict_trimm_rGT_notAE["Existence"][constructionDict_trimm_rGT_notAE["Dendrogram"]], constructionDict_trimm_rGT_notAE["AllLeaves"]),
                                                  len(constructionDict_trimm_rGT_notAE["AmbiguousEdges"][constructionDict_trimm_rGT_notAE["Dendrogram"]]))
...
# get consensus graphs of all experiments
# expand consensus graphs
consensus_AE_expand = getConsensusGraphs(constructionDict["Alignment"],
                                         constructionDict["Existence"][constructionDict["Dendrogram"]],
                                         constructionDict["AllLeaves"],
                                         kConsensusVals)
constructionDict["ConsensusGraphs"] = deepcopy(consensus_AE_expand)
constructionDict["kConsensusVals"] = deepcopy(kConsensusVals)


consensus_notAE_expand = getConsensusGraphs(constructionDict_notAE["Alignment"],
                                            constructionDict_notAE["Existence"][constructionDict_notAE["Dendrogram"]],
                                            constructionDict_notAE["AllLeaves"],
                                            kConsensusVals)
constructionDict_notAE["ConsensusGraphs"] = deepcopy(consensus_notAE_expand)
constructionDict_notAE["kConsensusVals"] = deepcopy(kConsensusVals)


consensus_rGT_AE_expand = getConsensusGraphs(constructionDict_rGT["Alignment"],
                                             constructionDict_rGT["Existence"][constructionDict_rGT["Dendrogram"]],
                                             constructionDict_rGT["AllLeaves"],
                                             kConsensusVals)
constructionDict_rGT["ConsensusGraphs"] = deepcopy(consensus_rGT_AE_expand)
constructionDict_rGT["kConsensusVals"] = deepcopy(kConsensusVals)


consensus_rGT_notAE_expand = getConsensusGraphs(constructionDict_rGT_notAE["Alignment"],
                                                constructionDict_rGT_notAE["Existence"][constructionDict_rGT_notAE["Dendrogram"]],
                                                constructionDict_rGT_notAE["AllLeaves"],
                                                kConsensusVals)
constructionDict_rGT_notAE["ConsensusGraphs"] = deepcopy(consensus_rGT_notAE_expand)
constructionDict_rGT_notAE["kConsensusVals"] = deepcopy(kConsensusVals)


# trimm consensus graphs
consensus_AE_trimm = getConsensusGraphs(constructionDict_trimm["Alignment"],
                                        constructionDict_trimm["Existence"][constructionDict_trimm["Dendrogram"]],
                                        constructionDict_trimm["AllLeaves"],
                                        kConsensusVals)
constructionDict_trimm["ConsensusGraphs"] = deepcopy(consensus_AE_trimm)
constructionDict_trimm["kConsensusVals"] = deepcopy(kConsensusVals)


consensus_notAE_trimm = getConsensusGraphs(constructionDict_trimm_notAE["Alignment"],
                                           constructionDict_trimm_notAE["Existence"][constructionDict_trimm_notAE["Dendrogram"]],
                                           constructionDict_trimm_notAE["AllLeaves"],
                                           kConsensusVals)
constructionDict_trimm_notAE["ConsensusGraphs"] = deepcopy(consensus_notAE_trimm)
constructionDict_trimm_notAE["kConsensusVals"] = deepcopy(kConsensusVals)


consensus_rGT_AE_trimm = getConsensusGraphs(constructionDict_trimm_rGT["Alignment"],
                                            constructionDict_trimm_rGT["Existence"][constructionDict_trimm_rGT["Dendrogram"]],
                                            constructionDict_trimm_rGT["AllLeaves"],
                                            kConsensusVals)
constructionDict_trimm_rGT["ConsensusGraphs"] = deepcopy(consensus_rGT_AE_trimm)
constructionDict_trimm_rGT["kConsensusVals"] = deepcopy(kConsensusVals)


consensus_rGT_notAE_trimm = getConsensusGraphs(constructionDict_trimm_rGT_notAE["Alignment"],
                                               constructionDict_trimm_rGT_notAE["Existence"][constructionDict_trimm_rGT_notAE["Dendrogram"]],
                                               constructionDict_trimm_rGT_notAE["AllLeaves"],
                                               kConsensusVals)
constructionDict_trimm_rGT_notAE["ConsensusGraphs"] = deepcopy(consensus_rGT_notAE_trimm)
constructionDict_trimm_rGT_notAE["kConsensusVals"] = deepcopy(kConsensusVals)


# task message
print("\n")
print("* Obtaining order of MCS between primal and consensus ...")


# get order of MCS between primal and consensus
condPrimal = deepcopy(condensedLabel(primal))


# kernel GT with expand and with ambiguous edges
tempArry = []
orderedGraphs = deepcopy([consensus_AE_expand[(eachK, totGraphs)] for (eachK, totGraphs) in kConsensusVals])
for eachGraph in orderedGraphs:
    condConsensus = deepcopy(condensedLabel(eachGraph))
    alignmentScoreResult, matchResult = pairwiseAlignment(condPrimal, condConsensus,
                                                          dict(), dict(),
                                                          [""], [""],
                                                          "undirected",
                                                          score = "order",
                                                          ...
                                                          eLabels = respectELabels,
                                                          algorithm = MCSalgorithm)
    tempArry.append(alignmentScoreResult)
constructionDict["PrimalConsensus"] = deepcopy(tempArry)
print("kGT - expand - AE")
print(constructionDict["PrimalConsensus"])


# kernel GT with expand and without ambiguous edges
tempArry = []
orderedGraphs = deepcopy([consensus_notAE_expand[(eachK, totGraphs)] for (eachK, totGraphs) in kConsensusVals])
for eachGraph in orderedGraphs:
    condConsensus = deepcopy(condensedLabel(eachGraph))
    alignmentScoreResult, matchResult = pairwiseAlignment(condPrimal, condConsensus,
                                                          dict(), dict(),
                                                          ...
                                                          algorithm = MCSalgorithm)
    tempArry.append(alignmentScoreResult)
constructionDict_notAE["PrimalConsensus"] = deepcopy(tempArry)
print("kGT - expand - nAE")
print(constructionDict_notAE["PrimalConsensus"])


# random GT with expand and with ambiguous edges
tempArry = []
orderedGraphs = deepcopy([consensus_rGT_AE_expand[(eachK, totGraphs)] for (eachK, totGraphs) in kConsensusVals])
for eachGraph in orderedGraphs:
    condConsensus = deepcopy(condensedLabel(eachGraph))
    alignmentScoreResult, matchResult = pairwiseAlignment(condPrimal, condConsensus,
                                                          dict(), dict(),
                                                          ...
                                                          algorithm = MCSalgorithm)
    tempArry.append(alignmentScoreResult)
constructionDict_rGT["PrimalConsensus"] = deepcopy(tempArry)
print("rGT - expand - AE")
print(constructionDict_rGT["PrimalConsensus"])


# random GT with expand and without ambiguous edges
tempArry = []
orderedGraphs = deepcopy([consensus_rGT_notAE_expand[(eachK, totGraphs)] for (eachK, totGraphs) in kConsensusVals])
for eachGraph in orderedGraphs:
    condConsensus = deepcopy(condensedLabel(eachGraph))
    alignmentScoreResult, matchResult = pairwiseAlignment(condPrimal, condConsensus,
                                                          dict(), dict(),
                                                          ...
                                                          algorithm = MCSalgorithm)
    tempArry.append(alignmentScoreResult)
constructionDict_rGT_notAE["PrimalConsensus"] = deepcopy(tempArry)
print("rGT - expand - nAE")
print(constructionDict_rGT_notAE["PrimalConsensus"])


# kernel GT with trimm and with ambiguous edges
tempArry = []
orderedGraphs = deepcopy([consensus_AE_trimm[(eachK, totGraphs)] for (eachK, totGraphs) in kConsensusVals])
for eachGraph in orderedGraphs:
    condConsensus = deepcopy(condensedLabel(eachGraph))
    alignmentScoreResult, matchResult = pairwiseAlignment(condPrimal, condConsensus,
                                                          dict(), dict(),
                                                          ...
                                                          algorithm = MCSalgorithm)
    tempArry.append(alignmentScoreResult)
constructionDict_trimm["PrimalConsensus"] = deepcopy(tempArry)
print("kGT - trimm - AE")
print(constructionDict_trimm["PrimalConsensus"])


# kernel GT with trimm and without ambiguous edges
tempArry = []
orderedGraphs = deepcopy([consensus_notAE_trimm[(eachK, totGraphs)] for (eachK, totGraphs) in kConsensusVals])
for eachGraph in orderedGraphs:
    condConsensus = deepcopy(condensedLabel(eachGraph))
    alignmentScoreResult, matchResult = pairwiseAlignment(condPrimal, condConsensus,
                                                          dict(), dict(),
                                                          ...
                                                          algorithm = MCSalgorithm)
    tempArry.append(alignmentScoreResult)
constructionDict_trimm_notAE["PrimalConsensus"] = deepcopy(tempArry)
print("kGT - trimm - nAE")
print(constructionDict_trimm_notAE["PrimalConsensus"])


# random GT with trimm and with ambiguous edges
tempArry = []
orderedGraphs = deepcopy([consensus_rGT_AE_trimm[(eachK, totGraphs)] for (eachK, totGraphs) in kConsensusVals])
for eachGraph in orderedGraphs:
    condConsensus = deepcopy(condensedLabel(eachGraph))
    alignmentScoreResult, matchResult = pairwiseAlignment(condPrimal, condConsensus,
                                                          dict(), dict(),
                                                          ...
                                                          algorithm = MCSalgorithm)
    tempArry.append(alignmentScoreResult)
constructionDict_trimm_rGT["PrimalConsensus"] = deepcopy(tempArry)
print("rGT - trimm - AE")
print(constructionDict_trimm_rGT["PrimalConsensus"])


# random GT with trimm and without ambiguous edges
tempArry = []
orderedGraphs = deepcopy([consensus_rGT_notAE_trimm[(eachK, totGraphs)] for (eachK, totGraphs) in kConsensusVals])
for eachGraph in orderedGraphs:
    condConsensus = deepcopy(condensedLabel(eachGraph))
    alignmentScoreResult, matchResult = pairwiseAlignment(condPrimal, condConsensus,
                                                          dict(), dict(),
                                                          ...
                                                          algorithm = MCSalgorithm)
    tempArry.append(alignmentScoreResult)
constructionDict_trimm_rGT_notAE["PrimalConsensus"] = deepcopy(tempArry)
print("rGT - trimm - nAE")
print(constructionDict_trimm_rGT_notAE["PrimalConsensus"])
...
allExperiments = (constructionDict,
                  ...
                  constructionDict_rGT,
                  constructionDict_rGT_notAE,
                  constructionDict_trimm,
                  constructionDict_trimm_notAE,
                  constructionDict_trimm_rGT,
                  constructionDict_trimm_rGT_notAE)
