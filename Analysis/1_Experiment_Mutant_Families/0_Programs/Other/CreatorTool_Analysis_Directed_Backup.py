...
totChildren = 2     # n-children of inner nodes produces n-ary tree
...
while(len(allPrimal) < nonIsoPrimals):
    ...
    for donePrimal in allPrimal:
        isomorphic = nx.is_isomorphic(newPrimal, donePrimal.to_undirected())
        if(isomorphic):
            break
    ...
    if(not isomorphic):
        ...
        for (u, v) in list(newPrimal.edges()):
            forward = random.choice(coinToss)
            if(forward):
                basicEdges.append((u, v))
            else:
                basicEdges.append((v, u))            
        newPrimal = deepcopy(newPrimal.to_directed())
        newPrimal.remove_edges_from(basicEdges)
        # reinitialize labels
        nodeLabel = dict()
        edgeLabel = dict()
        # label vertices
...
for eachPrimalDir in allPrimal:
    eachPrimal = deepcopy(eachPrimalDir.to_undirected())
...
for eachPrimal in allPrimal:
    ...
    for j in range(rateVertexAdd):
        ...
        mustLabel = random.choice(coinToss)
        ...
        for eachNeigh in neighbors:
            ...
            if(forward):
                ...
                newGraph.add_edge(eachNeigh, newNode, ePrimalLabel = random.choice(eLabels))
        newNode = newNode + 1
            ...
            for j in range(totChildren):
                ...
                while(isomorphic):
                    ...
                    for j in range(rateVertexAdd):
                        ...
                        # choose neighbors
                        neighbors = list(random.sample(list(newGraph.nodes()), newDeg))
                        # add new vertex with random label (if any)
                        newGraph.add_node(newNode, vPrimalLabel = random.choice(vLabels))
