...
# user input for drawing, functionalities and heuristics -----------------------
upperBound3D = 5
upperBound2D = 20
upperBound1D = 40
...
constructionDict_notAE_notH = dict()   # no ambiguous edges check and no ROFI
constructionDict_yesAE_yesH = dict()   # ambiguous edges check and ROFI
constructionDict_notAE_yesH = dict()   # no ambiguous edges check and ROFI
constructionDict_rGT = dict()          # random guide tree
...
def pairwiseAlignment(someG1, someG2,
    ...
    if(len(possibleMatches) > 0):
        ...
        if("Maximal" in valsROFI):
            match = [eachMatch for (eachMatch, eachROFI) in possibleMatches if(eachROFI == "Maximal")][0]
        else:
...
def getROFI(someG1, someG2, someMatch):
    ...
    nodeKs = {"symb": kroneckerDeltaOneV, "nsymb": kroneckerDeltaOneV, "mix": kroneckerDeltaTwoV}
    edgeKs = {"symb": kroneckerDeltaOneE, "nsymb": kroneckerDeltaOneE, "mix": kroneckerDeltaTwoE}
...
def undirMatchMCS(someG1, someG2,
    ...
    if(len(allMatches) == 0):
        if(len(someMatch) > 0):
            # save match and its ROFI or "Maximal" if match is maximal
        ...
        if(scoreNewMatch == scoreOldMatch):
            ...
            if(not set(someMatch) in allMatchesSet):
                ...
                if(len(someMatch) < expOrder):
                    ...
                    allMatches = allMatches + [(someMatch, "Maximal")]
                    if(vLabels and eLabels):
                        if(ambiguousPairsCheck):
                            foundSubIso = isSubIso(someG1, someG2, someMatch)
                            if(foundSubIso):
...
def dirMatchMCS(someG1, someG2,
    ...
    if(len(allMatches) == 0):
        if(len(someMatch) > 0):
            # save match and its ROFI or "Maximal" if match is (sub)graph isomorphism
            if(len(someMatch) < expOrder):
                allMatches = [(someMatch, getROFI(someG1, someG2, someMatch))]
                if(vLabels and eLabels):
                    if(isMaxColoredMatch(someG1, someG2, someMatch)):
...
def PGA(someType, someGT, someBaseDict, someDendro, someContained, someScore,
        someHoldV, someHoldE, someHoldHeuristic, someHoldAmbEdges):
    # local variables
    counter = 0
    heightGT = 0
    finalTime = 0
    ...
    allLeavesGraphs = deepcopy(alignments)   # saves only leaves
    allAlignments = deepcopy(alignments)     # saves leaves and all alignments
        ...
        for cherry in cherries:
            ...
            if(someType == "undirected"):
                ...
                possibleMatches, isMatchAlsoSubIso, isMatchAlsoMaxColored = undirMatchMCS(G1, G2,
                                                                                          ...
                                                                                          vLabels = someHoldV,
                                                                                          eLabels = someHoldE,
                                                                                          heuristic = someHoldHeuristic,
                                                                                          ambiguousPairsCheck = someHoldAmbEdges)
                # check if MCS is empty (possible when considering vertex labels)
                if(len(possibleMatches) > 0):
                    valsROFI = [eachROFI for (eachMatch, eachROFI) in possibleMatches]
                    # select match minimizing ROFI or being (sub)graph isomorphism
                ...
                alignments[cherry] = deepcopy(filterCherry(resutls[0], someType, vLabels = someHoldV, eLabels = someHoldE))
    ...
    theAlignmentResults = (deepcopy(finalAlignment),
                           ...
                           deepcopy(existence),
                           (someScore, finalScore, someHoldV, someHoldE),
...
# task message
print("* initializing dictionaries for experiments with heuristic variations ...")


# initialize dictinoaries for experiments with random trees
constructionDict_notAE_notH = deepcopy(constructionDict)
constructionDict_yesAE_yesH = deepcopy(constructionDict)
constructionDict_notAE_yesH = deepcopy(constructionDict)


# task message
print("* Progressive graph alignment - Ambiguous Edges and NO Heuristic")
# progressive graph alignment
alignmentResults = PGA(theType, guideTreeSSP,
                       constructionDict, dendrogramSSP,
                       containedGraphsSSP, requestedScore,
                       respectVLabels, respectELabels,
                       heuristicROFI, ambiguousEdges)
# unpack results
...
# task message
print("\n")
print("* Progressive graph alignment - NO Ambiguous Edges and NO Heuristic")
# progressive graph alignment
alignmentResults = PGA(theType, guideTreeSSP,
                       constructionDict_notAE_notH, dendrogramSSP,
                       containedGraphsSSP, requestedScore,
                       respectVLabels, respectELabels,
                       False, False)
# unpack results
constructionDict_notAE_notH["Alignment"] = deepcopy(alignmentResults[0])
constructionDict_notAE_notH["allAlignments"] = deepcopy(alignmentResults[1])
constructionDict_notAE_notH["Existence"] = deepcopy(alignmentResults[2])
constructionDict_notAE_notH["ScoreInfo"] = deepcopy(alignmentResults[3])
constructionDict_notAE_notH["AmbiguousEdges"] = deepcopy(alignmentResults[4])
constructionDict_notAE_notH["MapToRoot"] = deepcopy(alignmentResults[5])
constructionDict_notAE_notH["AllLeaves"] = deepcopy(alignmentResults[6])
constructionDict_notAE_notH["AllLeavesGraphs"] = deepcopy(alignmentResults[7])
constructionDict_notAE_notH["Time"] = deepcopy(alignmentResults[8])


# task message
print("\n")
print("* Progressive graph alignment - Ambiguous Edges and Heuristic")
# progressive graph alignment
alignmentResults = PGA(theType, guideTreeSSP,
                       constructionDict_yesAE_yesH, dendrogramSSP,
                       containedGraphsSSP, requestedScore,
                       respectVLabels, respectELabels,
                       True, True)
# unpack results
constructionDict_yesAE_yesH["Alignment"] = deepcopy(alignmentResults[0])
constructionDict_yesAE_yesH["allAlignments"] = deepcopy(alignmentResults[1])
constructionDict_yesAE_yesH["Existence"] = deepcopy(alignmentResults[2])
constructionDict_yesAE_yesH["ScoreInfo"] = deepcopy(alignmentResults[3])
constructionDict_yesAE_yesH["AmbiguousEdges"] = deepcopy(alignmentResults[4])
constructionDict_yesAE_yesH["MapToRoot"] = deepcopy(alignmentResults[5])
constructionDict_yesAE_yesH["AllLeaves"] = deepcopy(alignmentResults[6])
constructionDict_yesAE_yesH["AllLeavesGraphs"] = deepcopy(alignmentResults[7])
constructionDict_yesAE_yesH["Time"] = deepcopy(alignmentResults[8])


# task message
print("\n")
print("* Progressive graph alignment - NO Ambiguous Edges and Heuristic")
# progressive graph alignment
alignmentResults = PGA(theType, guideTreeSSP,
                       constructionDict_notAE_yesH, dendrogramSSP,
                       containedGraphsSSP, requestedScore,
                       respectVLabels, respectELabels,
                       True, False)
# unpack results
constructionDict_notAE_yesH["Alignment"] = deepcopy(alignmentResults[0])
constructionDict_notAE_yesH["allAlignments"] = deepcopy(alignmentResults[1])
constructionDict_notAE_yesH["Existence"] = deepcopy(alignmentResults[2])
constructionDict_notAE_yesH["ScoreInfo"] = deepcopy(alignmentResults[3])
constructionDict_notAE_yesH["AmbiguousEdges"] = deepcopy(alignmentResults[4])
constructionDict_notAE_yesH["MapToRoot"] = deepcopy(alignmentResults[5])
constructionDict_notAE_yesH["AllLeaves"] = deepcopy(alignmentResults[6])
constructionDict_notAE_yesH["AllLeavesGraphs"] = deepcopy(alignmentResults[7])
constructionDict_notAE_yesH["Time"] = deepcopy(alignmentResults[8])


# task message
print("\n")
print("* Progressive graph alignment - Random Experiment")
# progressive graph alignment
...
# data for plots of main alignment
...
allExperiments = (constructionDict,
                  constructionDict_notAE_notH,
                  constructionDict_yesAE_yesH,
                  constructionDict_notAE_yesH,
