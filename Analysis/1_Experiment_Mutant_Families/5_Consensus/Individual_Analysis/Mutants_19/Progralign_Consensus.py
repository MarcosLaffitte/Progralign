################################################################################
#                                                                              #
#  README - Program: Progralign_Consensus.py                                   #
#                                                                              #
#  --------------------------------------------------------------------------  #
#                                                                              #
# - LICENSE:                                                                   #
...
inputFileNameResults = ""
inputFileNameResults = argv[2]
inputList = []
inputFile = None
drawingMode = False
...
constructionDict_rGTX1 = dict()                # expand over extra random guide tree 1 with ambiguous edges
constructionDict_rGTX1_notAE = dict()          # expand over extra random guide tree 1 without ambiguous edges
constructionDict_rGTX2 = dict()                # expand over extra random guide tree 2 with ambiguous edges
constructionDict_rGTX2_notAE = dict()          # expand over extra random guide tree 2 without ambiguous edges
# trimm-experiments
...
constructionDict_trimm_rGTX1 = dict()          # trimm over random guide tree 1 with ambiguous edges
constructionDict_trimm_rGTX1_notAE = dict()    # trimm over random guide tree 1 without ambiguous edges
constructionDict_trimm_rGTX2 = dict()          # trimm over random guide tree 2 with ambiguous edges
constructionDict_trimm_rGTX2_notAE = dict()    # trimm over random guide tree 2 without ambiguous edges


# dictionaries for consensus graphs --------------------------------------------
kConsensusVals = []
# expand consensus graphs
consensus_AE_expand = dict()
consensus_notAE_expand = dict()
consensus_rGT_AE_expand = dict()
consensus_rGT_notAE_expand = dict()
# trimm consensus graphs
consensus_AE_trimm = dict()
consensus_notAE_trimm = dict()
consensus_rGT_AE_trimm = dict()
consensus_rGT_notAE_trimm = dict()


# list of similarities between consensus graphs and primal ---------------------
kernel_similarities_expand = dict()
kernel_similarities_trimm = dict()
score_similarities_expand = dict()
score_similarities_trimm = dict()
...
# initial message
print("\n")
print(">>> Progralign Consensus - Progralign Github Repository")
...
# retrieve input results file
inputFile = open(inputFileNameResults, "rb")
inputTuple = pkl.load(inputFile)
inputFile.close()
...
# get primal and input list of graphs
# unpack expand dictionaries
constructionDict = deepcopy(inputTuple[0])
constructionDict_notAE = deepcopy(inputTuple[1])
constructionDict_rGT = deepcopy(inputTuple[2])
constructionDict_rGT_notAE = deepcopy(inputTuple[3])
# unpack trimm dictionaries
constructionDict_trimm = deepcopy(inputTuple[8])
constructionDict_trimm_notAE = deepcopy(inputTuple[9])
constructionDict_trimm_rGT = deepcopy(inputTuple[10])
constructionDict_trimm_rGT_notAE = deepcopy(inputTuple[11])
...
# # task message
# print("\n")
# print("* Obtaining kernel-similarites between primal and consensus ...")

# # get kernel-distance between primal and consensus
# # similarities for expand
# orderedGraphs = deepcopy([consensus_AE_expand[(eachK, totGraphs)] for (eachK, totGraphs) in kConsensusVals])
# kernel_similarities_expand["kernel-based GT and ambiguous edges"] = getSimilarities(primal, orderedGraphs)
# print(kernel_similarities_expand["kernel-based GT and ambiguous edges"])

# orderedGraphs = deepcopy([consensus_notAE_expand[(eachK, totGraphs)] for (eachK, totGraphs) in kConsensusVals])
# kernel_similarities_expand["kernel-based GT and NO ambiguous edges"] = getSimilarities(primal, orderedGraphs)
# print(kernel_similarities_expand["kernel-based GT and NO ambiguous edges"])

# orderedGraphs = deepcopy([consensus_rGT_AE_expand[(eachK, totGraphs)] for (eachK, totGraphs) in kConsensusVals])
# kernel_similarities_expand["random GT and ambiguous edges"] = getSimilarities(primal, orderedGraphs)
# print(kernel_similarities_expand["random GT and ambiguous edges"])

# orderedGraphs = deepcopy([consensus_rGT_notAE_expand[(eachK, totGraphs)] for (eachK, totGraphs) in kConsensusVals])
# kernel_similarities_expand["random GT and NO ambiguous edges"] = getSimilarities(primal, orderedGraphs)
# print(kernel_similarities_expand["random GT and NO ambiguous edges"])

# print("------------------------------------------------------------------------")

# # trimm consensus graphs
# orderedGraphs = deepcopy([consensus_AE_trimm[(eachK, totGraphs)] for (eachK, totGraphs) in kConsensusVals])
# kernel_similarities_trimm["kernel-based GT and ambiguous edges"] = getSimilarities(primal, orderedGraphs)
# print(kernel_similarities_trimm["kernel-based GT and ambiguous edges"])

# orderedGraphs = deepcopy([consensus_notAE_trimm[(eachK, totGraphs)] for (eachK, totGraphs) in kConsensusVals])
# kernel_similarities_trimm["kernel-based GT and NO ambiguous edges"] = getSimilarities(primal, orderedGraphs)
# print(kernel_similarities_trimm["kernel-based GT and NO ambiguous edges"])

# orderedGraphs = deepcopy([consensus_rGT_AE_trimm[(eachK, totGraphs)] for (eachK, totGraphs) in kConsensusVals])
# kernel_similarities_trimm["random GT and ambiguous edges"] = getSimilarities(primal, orderedGraphs)
# print(kernel_similarities_trimm["random GT and ambiguous edges"])

# orderedGraphs = deepcopy([consensus_rGT_notAE_trimm[(eachK, totGraphs)] for (eachK, totGraphs) in kConsensusVals])
# kernel_similarities_trimm["random GT and NO ambiguous edges"] = getSimilarities(primal, orderedGraphs)
# print(kernel_similarities_trimm["random GT and NO ambiguous edges"])


# task message
print("\n")
print("* Obtaining score-similarites between primal and consensus ...")


# get score-distance between primal and consensus
...
# score_similarities_expand["kernel GT and ambiguous edges"] = deepcopy(tempArry)
# print(score_similarities_expand["kernel GT and ambiguous edges"])
...
for eachGraph in orderedGraphs:
    ...
    alignmentScoreResult, matchResult = pairwiseAlignment(condPrimal, condConsensus,
                                                          ...
                                                          algorithm = MCSalgorithm)
    tempArry.append(alignmentScoreResult)
score_similarities_expand["kernel GT and NO ambiguous edges"] = deepcopy(tempArry)
print(score_similarities_expand["kernel GT and NO ambiguous edges"])
...
#                                                           algorithm = MCSalgorithm)
#     tempArry.append(alignmentScoreResult)
# score_similarities_expand["random GT and ambiguous edges"] = deepcopy(tempArry)
# print(score_similarities_expand["random GT and ambiguous edges"])
...
for eachGraph in orderedGraphs:
    ...
    alignmentScoreResult, matchResult = pairwiseAlignment(condPrimal, condConsensus,
                                                          ...
                                                          algorithm = MCSalgorithm)
    tempArry.append(alignmentScoreResult)
score_similarities_expand["random GT and NO ambiguous edges"] = deepcopy(tempArry)
print(score_similarities_expand["random GT and NO ambiguous edges"])
...
#                                                           algorithm = MCSalgorithm)
#     tempArry.append(alignmentScoreResult)
# score_similarities_trimm["kernel GT and ambiguous edges"] = deepcopy(tempArry)
# print(score_similarities_trimm["kernel GT and ambiguous edges"])
...
for eachGraph in orderedGraphs:
    ...
    alignmentScoreResult, matchResult = pairwiseAlignment(condPrimal, condConsensus,
                                                          ...
                                                          algorithm = MCSalgorithm)
    tempArry.append(alignmentScoreResult)
score_similarities_trimm["kernel GT and NO ambiguous edges"] = deepcopy(tempArry)
print(score_similarities_trimm["kernel GT and NO ambiguous edges"])
...
#                                                           algorithm = MCSalgorithm)
#     tempArry.append(alignmentScoreResult)
# score_similarities_trimm["random GT and ambiguous edges"] = deepcopy(tempArry)
# print(score_similarities_trimm["random GT and ambiguous edges"])
...
for eachGraph in orderedGraphs:
    ...
    alignmentScoreResult, matchResult = pairwiseAlignment(condPrimal, condConsensus,
                                                          ...
                                                          algorithm = MCSalgorithm)
    tempArry.append(alignmentScoreResult)
score_similarities_trimm["random GT and NO ambiguous edges"] = deepcopy(tempArry)
print(score_similarities_trimm["random GT and NO ambiguous edges"])


# make line plots of similarities ----------------------------------------------
allLines = ["kernel GT and NO ambiguous edges",
            "random GT and NO ambiguous edges"]
...
X = [1, 2, 3, 4, 5, 6, 7]
kConsensusValsStr = [str(eachK) + "/" + str(totGraphs) for (eachK, totGraphs) in kConsensusVals]


# make plot of score similarities for expand
for i in range(0, len(allLines)):
    newHandle = plt.plot(X, score_similarities_expand[allLines[i]],
                         linewidth = 1, color = allColors[i],
                         marker = "o", markersize = 3,
                         linestyle = allStyles[i],
                         label = allLabels[i])
    plt.xticks([])
    plt.xticks(X, labels = kConsensusValsStr)
    plt.xlabel("Proportion")
    plt.ylabel("Order of MCS")
plt.title("score-expand")
plt.legend(fontsize = 7)
plt.tight_layout()
plt.savefig("score_expand.pdf")
plt.close()


# make plot of score similarities for trimm
for i in range(0, len(allLines)):
    newHandle = plt.plot(X, score_similarities_trimm[allLines[i]],
                         linewidth = 1, color = allColors[i],
                         marker = "o", markersize = 3,
                         linestyle = allStyles[i],
                         label = allLabels[i])
    ...
    plt.xlabel("Proportion")
    plt.ylabel("Order of MCS")
plt.title("score-trimm")
plt.legend(fontsize = 7)
plt.tight_layout()
plt.savefig("score_trimm.pdf")
plt.close()
...
kernelDistancePMVals = deepcopy(constructionDict["KernelDistancePMVals"])
scoreDistancePMVals = deepcopy(constructionDict["ScoreDistancePMVals"])
kernelDistanceMMVals = deepcopy(constructionDict["KernelDistanceMMVals"])
scoreDistanceMMVals = deepcopy(constructionDict["ScoreDistanceMMVals"])
